...
> \[참고\] 쓰레드의 실행순서는 **OS의 스케쥴러가 작성한 스케쥴에 의해 결정**됨
...
## 3. start()와 run()
![run method](https://github.com/rlawjddbs/rlawjddbs.github.io/blob/master/_posts/imgs/201028/thread.png)   
- 쓰레드를 실행시킬 때 run()이 아닌 start()를 호출하는 이유
    - main 메서드에서 run()을 호출하는 것은 생성된 쓰레드를 실행시키는 것이 아니라 단순히 클래스에 선언된 메서드를 호출하는 것일 뿐임

![run method](https://github.com/rlawjddbs/rlawjddbs.github.io/blob/master/_posts/imgs/201028/run_and_start.png)   
